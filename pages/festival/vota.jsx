...
const Vota = ({ cortometraggi = [], error = null }) => {
      ...
      if (isAuthenticated && user) {
        try {
          ...
          if (!session?.access_token) {
            throw new Error('Token di sessione non disponibile');
          }

          const userVotes = await getUserVotesFromAPI(session.access_token);
          const votesMap = {};
          userVotes.forEach(vote => {
            votesMap[vote.film_id] = vote.rating;
          });
  ...
  const handleRatingChange = async (cortoId, newRating) => {
    ...
    try {
      ...
      // Salva tramite API
      await saveVoteToAPI(session.access_token, cortoId, newRating);
