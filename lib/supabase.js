...
export const getDetailedVotesStats = async (filters = {}) => {
  ...
  if (isDemoMode) {
    let filteredVotes = [...demoVotes];
    
    // Applica filtri
    if (dateFrom) {
      filteredVotes = filteredVotes.filter(vote => new Date(vote.created_at) >= new Date(dateFrom));
    }
    if (dateTo) {
      filteredVotes = filteredVotes.filter(vote => new Date(vote.created_at) <= new Date(dateTo));
    }
    if (filmId) {
      filteredVotes = filteredVotes.filter(vote => vote.film_id === filmId);
    }
    if (minRating) {
      filteredVotes = filteredVotes.filter(vote => vote.rating >= minRating);
    }
    if (maxRating) {
      filteredVotes = filteredVotes.filter(vote => vote.rating <= maxRating);
    }
    
    // Calcola statistiche per film
    const filmStats = filmData.map(film => {
    ...
    return {
      ...
      filmStats: filmStats.sort((a, b) => b.averageRating - a.averageRating),
      topFilms: filmStats.slice(0, 5),
      recentVotes: filteredVotes.slice(-10).reverse()
    };
  }
  
  // Implementazione per database reale
  // ... codice per database reale
};
