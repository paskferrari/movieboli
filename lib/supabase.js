...
export const getAllVotesDetailed = async () => {
  if (isDemoMode) {
    // Simulazione voti dettagliati per modalità demo
    return demoVotes.map(vote => ({
      ...vote,
      user_profiles: {
        id: vote.user_id,
        full_name: vote.user_id === demoUser.id ? demoUser.user_metadata.full_name : 
                   vote.user_id === demoAdmin.id ? demoAdmin.user_metadata.full_name : 
                   `Demo User ${vote.user_id.slice(-3)}`,
        email: vote.user_id === demoUser.id ? demoUser.email : 
               vote.user_id === demoAdmin.id ? demoAdmin.email : 
               `user${vote.user_id.slice(-3)}@demo.com`,
...
export const getVotesDashboardStats = async () => {
  if (isDemoMode) {
    // Simulazione statistiche per modalità demo
    const votes = demoVotes;
